    ...
    <PageWrapper>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <button
          ...
          className="p-4 rounded-xl border bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200 text-left hover:shadow-md transition-shadow"
        >
        ...
        <button
          ...
          className="p-4 rounded-xl border bg-gradient-to-br from-green-50 to-green-100 border-green-200 text-left hover:shadow-md transition-shadow"
        >
          <div className="w-10 h-10 rounded-full bg-green-500 flex items-center justify-center mb-3">
            <Phone size={20} className="text-white" />
          </div>
          <p className="font-semibold text-sm">Call County</p>
          <p className="text-xs text-muted-foreground mt-1">{PINELLAS_COUNTY_BUILDING.phone}</p>
        </button>
      </div>

      {/* County Hours */}
      <div className="p-3 rounded-lg bg-muted mb-6 flex items-center gap-3">
        ...
        <div className="space-y-2">
          {quickQuestions.map((question) => (
            <button
              key={question}
              onClick={() => navigate("/ai-assistant")}
              className="w-full text-left p-3 rounded-lg bg-card border hover:bg-muted/50 transition-colors flex items-center justify-between"
            >
      ...
      <section className="mb-6">
        ...
        <div className="space-y-2">
          {videoGuides.map((video) => (
            <button
              key={video.title}
              className="w-full text-left p-3 rounded-lg bg-card border hover:bg-muted/50 transition-colors flex items-center gap-3"
            >
      ...
      <button
        ...
        className="w-full p-4 rounded-xl border bg-card hover:bg-muted/50 transition-colors flex items-center gap-3"
      >
        ...
        <div className="flex-1 text-left">
          ...
          <p className="text-sm text-muted-foreground">support@permitpath.com</p>
        </div>
        <ChevronRight size={16} className="text-muted-foreground" />
      </button>
    </PageWrapper>
